...
export default function Electronics() {
  ...
  useEffect(() => {
    ...
    getAllProducts()
      .then((p) => {
        if (mounted) {
          // Filter to only show laptops and mobile-accessories
          const electronics = p.filter(
            (product) =>
  ...
  return (
    <section className="container">
      ...
      <p className="category-description">
        Discover our curated selection of laptops and mobile accessories.
        Find the perfect tech to enhance your digital lifestyle.
      </p>
      <FiltersBar products={products} state={filters} setState={setFilters} />
      {loading ? <div className="muted">Loading electronics...</div> : null}
      {!loading && filtered.length === 0 ? (
        <div className="muted">No electronics found with these filters.</div>
      ) : null}
